    ...
    internal sealed class TrialAndErrorRemoval
    {
        ...
        public async Task<string> StartAsync(VCFile file, TrialAndErrorRemovalOptions settings)
        {
            ...
            if (!await TestCompileAsync(config))
                return $"{document.FilePath} failed to compile. Include removal stopped.";

            // Skip first on demand
            var iterator = (settings.IgnoreFirstInclude ? lines.Skip(1) : lines.AsEnumerable())
                .Where(s => !s.Keep); // filter kept
