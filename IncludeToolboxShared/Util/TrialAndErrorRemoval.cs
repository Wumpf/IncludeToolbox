...
namespace IncludeToolbox
{
    ...
    internal sealed class TrialAndErrorRemoval
    {
        ...
        public async Task<bool> PerformTrialAndErrorIncludeRemovalAsync(EnvDTE.Document document)
        {
            ...
            {
                _ = Output.WriteLineAsync($"Can't compile file '{canCompile.Reason}': {document.Name}");
                return false;
            }

            if (WorkInProgress)
            {
                _ = VS.MessageBox.ShowErrorAsync("Trial and error include removal already in progress!");
                return false;
            }
            WorkInProgress = true;
            ...
            {
                _=Output.WriteLineAsync($"Unexpected error while extracting include selection: {ex}");
        ...
        private async Task<IVsThreadedWaitDialog2> StartProgressDialog(string documentName)
        {
            ...
            {
                Output.WriteLine("Failed to get Dialog Factory for wait dialog.");
                return null;
            }
            ...
            if (progressDialog == null)
            {
                Output.WriteLine("Failed to get create wait dialog.");
                return null;
            }
        ...
        private void ExtractSelectionAndIncludes(EnvDTE.Document document, TrialAndErrorRemovalOptions settings,
                                                out ITextBuffer textBuffer, out Formatter.IncludeLineInfo[] includeLinesArray)
        {
            ...
            if (settings.RemovalOrder == IncludeRemovalOrder.BottomToTop)
                includeLines = includeLines.Reverse();

            includeLinesArray = includeLines.ToArray();
        }

        private void TrialAndErrorRemovalThreadFunc(EnvDTE.Document document, TrialAndErrorRemovalOptions settings,
                                                    Formatter.IncludeLineInfo[] includeLines, IVsThreadedWaitDialog2 progressDialog, ITextBuffer textBuffer)
        {
            ...
            try
            {
                ...
                foreach (Formatter.IncludeLineInfo line in includeLines)
                {
                    ...
                    if (settings.RemovalOrder == IncludeRemovalOrder.TopToBottom)
                    ...
                    ThreadHelper.JoinableTaskFactory.Run(async () =>
                    {
                        ...
                        for (int numCompileFails = 0; numCompileFails < maxNumCompileAttempts; ++numCompileFails)
                        {
                            ...
                            {
                                _=Output.WriteLineAsync($"Compile Failed:\n{e}");
                    ...
                    {
                        Output.WriteLine($"Could not remove #include: '{line.IncludeContent}'");
                        ThreadHelper.JoinableTaskFactory.Run(async () =>
                        {
                            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
                            document.Undo();
                            if (!noTimeout)
                                await VS.MessageBox.ShowErrorAsync($"Compilation of {document.Name} timeouted!");
                        });
                    ...
                    {
                        Output.WriteLine($"Successfully removed #include: '{line.IncludeContent}'");
                        ++numRemovedIncludes;
                    }
                }
            }
            ...
            {
                Output.WriteLine($"Unexpected error: {ex}");
            }
            finally
            {
        ...
        private async Task OnTrialAndErrorRemovalDone(IVsThreadedWaitDialog2 progressDialog, EnvDTE.Document document, int numRemovedIncludes, bool canceled)
        {
            ...
            // Message.
            Output.WriteLine(string.Format("Removed {0} #include directives from '{1}'", numRemovedIncludes, document.Name));
            _=Output.BringForwardAsync();
        ...
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            buildEvents = VCUtil.GetDTE().Events.BuildEvents;
        ...
        {
            //Output.WriteLine("OnBuildFinished. scope: {0}, action: {1}", scope, action);
            outputWaitEvent.Set();
        }
        ...
        {
            //Output.WriteLine("OnBuildConfigFinished. project {0}, projectConfig {1}, platform {2}, solutionConfig {3}, success {4}", project, projectConfig, platform, solutionConfig, success);
